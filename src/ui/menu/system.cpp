...
static void brightness_slider_event(lv_event_t *e)
{
    ...
    globalSettings.brightness = v;
    if (lv_obj_is_valid(ui_LabelBrightnessVal))
        lv_label_set_text_fmt(ui_LabelBrightnessVal, "%d", v);
}
static void card_menu_system_construct(lv_obj_t *parent)
{
    ...
    lv_obj_t *ui_LabelBrightness = lv_label_create(parent);
    lv_obj_set_width(ui_LabelBrightness, LV_SIZE_CONTENT);
    lv_obj_set_height(ui_LabelBrightness, LV_SIZE_CONTENT);
    lv_obj_set_pos(ui_LabelBrightness, 5, 154);
    lv_label_set_text(ui_LabelBrightness, "BL");
    lv_obj_set_style_text_font(ui_LabelBrightness, &ui_font_chinese16, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_SliderBrightness = lv_slider_create(parent);
    lv_slider_set_range(ui_SliderBrightness, 0, 255);
    lv_slider_set_value(ui_SliderBrightness, globalSettings.brightness, LV_ANIM_OFF);
    lv_obj_set_size(ui_SliderBrightness, 190, 20);
    lv_obj_set_pos(ui_SliderBrightness, 55, 150);
    lv_obj_add_flag(ui_SliderBrightness, LV_OBJ_FLAG_SCROLL_ON_FOCUS);
    lv_obj_add_event_cb(ui_SliderBrightness, brightness_slider_event, LV_EVENT_VALUE_CHANGED, NULL);

    ui_LabelBrightnessVal = lv_label_create(parent);
    lv_obj_set_width(ui_LabelBrightnessVal, LV_SIZE_CONTENT);
    lv_obj_set_height(ui_LabelBrightnessVal, LV_SIZE_CONTENT);
    lv_obj_set_pos(ui_LabelBrightnessVal, 255, 154);
    lv_label_set_text_fmt(ui_LabelBrightnessVal, "%d", globalSettings.brightness);
    lv_obj_set_style_text_font(ui_LabelBrightnessVal, &ui_font_chinese16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_add_event_cb(ui_Button6, [](lv_event_t *e)
        ...
        if (factory_reset_cnt == 0)
        {
            ...
            if (lv_obj_is_valid(ui_SliderBrightness))
                lv_slider_set_value(ui_SliderBrightness, globalSettings.brightness, LV_ANIM_OFF);
            if (lv_obj_is_valid(ui_LabelBrightnessVal))
                lv_label_set_text_fmt(ui_LabelBrightnessVal, "%d", globalSettings.brightness);
